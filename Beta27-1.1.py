...
from PyQt6.QtGui import QPalette, QColor, QIntValidator, QIcon, QAction
...
def resource_path(relative_path):
    """Получение пути к ресурсам, поддержка PyInstaller"""
    try:
...
def write_log(text: str):
    """Запись текста с меткой времени в лог"""
...
def event_to_dict(event):
    """Конвертация события Windows Event в словарь для логирования"""
    d = {}
...
class TriggerSettingsForm(QDialog):
    ...
    def __init__(self):
        ...
        # Обновляем состояние контролов по настройкам
    ...
    def save_config(self):
        ...
        try:
            ...
            # Если есть колбек — вызываем его для обновления настроек в основном окне
...
class WheaMonitorApp(QWidget):
    """Главное окно приложения с мониторингом WHEA событий"""
    ...
    def __init__(self):
        ...
        # Защита от запуска второй копии приложения (shared memory)
        ...
        self.tray_message_shown = False  # Показывать уведомление о сворачивании один раз

        # Таймер для периодической проверки событий
        self.timer = QTimer()
        self.timer.timeout.connect(self.check_whea_events)

        # Создаем диалог триггера и задаем callback на обновление настроек
        ...
        # Хранит текущую конфигурацию триггера (загружается из файла)
        self.trigger_config = {}
    ...
    def setup_ui(self):
        ...
        # Кнопка открытия настроек триггера (модальное окно)
        ...
        # Кнопка запуска bat-утилиты (WHEA Tools)
    ...
    def setup_tray(self):
        """Создание системного трея и меню"""
        self.tray_icon = QSystemTrayIcon(self.icon, self)
        tray_menu = QMenu()

        # Заголовок меню (активен, разворачивает окно)
        ...
        # Запуск/Остановка мониторинга из трея
    ...
    def on_tray_icon_activated(self, reason):
        """Открытие окна по клику на иконку в трее"""
        if reason == QSystemTrayIcon.ActivationReason.Trigger:
            self.show_normal()

    def show_normal(self):
        """Показать и активировать основное окно"""
        self.show()
        self.raise_()
        self.activateWindow()

    def closeEvent(self, event):
        """Перехват закрытия окна — скрываем и показываем уведомление"""
        event.ignore()
        self.hide()
    ...
    def update_tray_actions(self):
        """Обновление меню трея в зависимости от статуса таймера"""
    ...
    def check_whea_events(self):
        """Проверка событий WHEA в системном журнале Windows"""
        server = 'localhost'
        log_type = 'System'
    ...
    def handle_trigger(self):
        """Выполнение действий при срабатывании триггера (сообщение, звук, запуск)"""
        ...
        if cfg.get("execute_enabled", False):
            ...
            if path and os.path.exists(path):
                try:
                    # Используем os.startfile с аргументами корректно
                    if args.strip():
                        # os.startfile не поддерживает аргументы напрямую, используйте subprocess
    ...
    def update_trigger_config(self, config):
        """Обновление текущей конфигурации триггера из диалога"""
    ...
    def run_whea_tools(self):
        """Запуск bat-скрипта WHEA Tools в отдельном процессе"""
        bat_code = r"""@echo off
chcp 65001 >nul
